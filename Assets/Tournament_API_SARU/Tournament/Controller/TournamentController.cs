...
public class TournamentController
{
    ...
    [SerializeField]
    public List<TournamentModel> LiveTournamentList = new List<TournamentModel>();
    ...
    public void BuyTicketsForTournament(string tournament_id, string round_id, string session_token, TicketList ticket_data, int amount, string userToken, string currency, string meta_data, Action<bool, string> action ,string user_id)
    {
        Dictionary<string, string> parameters = new Dictionary<string, string>();
        List<TicketList> tickets = new List<TicketList>();
        tickets.Add(ticket_data);
        ...
        parameters.Add("ticket_data", JsonConvert.SerializeObject(tickets));
        ...
        WebApiManager.Instance.GetNetWorkCallPostMethodUsingJson(APIURL, body, (success, error, body) =>
        {
            if (success)
            {
                ...
                FetchTournament(APIURL, user_id, tournament_id, (success, updatedtournament) =>
                {

                    Debug.Log("updated tournament data after bet is "+JsonUtility.ToJson(updatedtournament));
                    ...
                    TournamentManager.instance.OnUpdateSingleTournamentUpdate[TournamentList[index].tournament_id]?.Invoke(TournamentList[index]);
    ...
    public void CancelTournamentTicket(string ticket_id, string tournament_id, string session_token, string userToken, string currency, Action<bool, string> action , string user_id)
    {
        Dictionary<string, string> parameters = new Dictionary<string, string>();
    ...
    public void ValidateTournament(DateTime dateTime)
    {
        ...
        foreach (var tournament in tournament_list)
        {
            ...
            .Where(t => t.round_data[1].status == TournamentStatus.Live || t.round_data[0].status == TournamentStatus.Live)
            .OrderBy(t => t.round_data[1].closed_at)
            .FirstOrDefault();
                ...
                if (TournamentList[nextTournamentIndex].status != TournamentStatus.Live)
                {
                    ...
                    TournamentManager.instance.OnUpdateSingleTournamentUpdate[TournamentList[nextTournamentIndex].tournament_id]?.Invoke(TournamentList[nextTournamentIndex]);
                }


                Debug.Log("invoke tournamnet changes :::  single tournament update" + TournamentList[index].id);
            });
        }
        ...
        WebApiManager.Instance.GetNetWorkCallPostMethodUsingJson(url, body, (success, error, body) =>
        {
            if (success)
            {
                ...
                foreach (var tournament in TournamentList)
                {
                    ...
                    if (!TournamentManager.instance.OnUpdateSingleTournamentUpdate.ContainsKey(tournament.tournament_id))
                    {
                        TournamentManager.instance.OnUpdateSingleTournamentUpdate.Add(tournament.tournament_id, tournament.action);
                    }
                    else
                    {
                        Debug.Log($"check this tournament with backend team... tournament_id  {tournament.tournament_id}   tournament_name  {tournament.tournament_name}  reson tournament duplicate");
                ...
                LiveTournamentList = TournamentList
                .Where(t => t.round_data[0].ticket.total_count + t.round_data[1].ticket.total_count != 0)
                .OrderBy(t => t.round_data[0].closed_at)
                .ToList();
                TournamentModel nextTournament = TournamentList
                .Where(t => t.round_data[1].status == TournamentStatus.Live || t.round_data[0].status == TournamentStatus.Live)
                .OrderBy(t => t.round_data[0].closed_at).FirstOrDefault();
                int nextTournamentIndex = TournamentList.FindIndex(x => x.tournament_id == nextTournament.tournament_id);

                Debug.Log("load tournament" + nextTournamentIndex);
    ...
    {
        TournamentList[86].action?.Invoke(TournamentList[86]);
        Debug.Log("invoke tournamnet changes :::  single tournament update" + TournamentList[86].tournament_id);
    }
